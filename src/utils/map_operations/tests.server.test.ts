...
import { SELECTION_NONE } from "@/stores/jotai";
import { getExtent } from "@/utils/geometry";
import { decodeId } from "@/utils/id";
import {
  deleteFeatures,
  removeCoordinatesVertex,
} from "@/utils/map_operations/delete_features";
import { duplicateFeatures } from "@/utils/map_operations/duplicate_features";
import { EMPTY_MOMENT } from "@/utils/persistence/moment";
import { generateSyntheticPoints } from "@/utils/pmap/generate_synthetic_points";
import {
  addInnerRing,
  CanInnerRingResult,
  canInnerRing,
...
describe("map_operations", () => {
  ...
  describe("mergeFeatures", () => {
    ...
    it("Polygon", () => {
      ...
      expect(
        mergeFeatures([...fcMultiPoly.features, ...fcPoly.features]),
  ...
  it("polygonToLine", () => {
    expect(
      polygonToLine(fcPoly.features[0] as IFeature<Polygon>),
    ).toMatchInlineSnapshot(`
      {
        "geometry": {
          "coordinates": [
    ...
    `);
    expect(
      polygonToLine(fcMultiPoly.features[0] as IFeature<MultiPolygon>),
    ).toMatchInlineSnapshot(`
      {
        "geometry": {
          "coordinates": [
  ...
  it("removeCoordinatesVertex", () => {
    for (const f of featureMap.values()) {
      ...
      for (const id of vertexIds) {
        ...
        if (res) {
          expect(geojsonhint.hint(JSON.stringify(res))).toHaveLength(0);
        }
      }
    }
  });
  ...
  describe("inner ring-3", () => {
    ...
    describe("addInnerRing", () => {
      it("throws if a polygon is not fully contained", () => {
        expect(
          addInnerRing(
            fcTwoPoly.features[0] as any,
            fcTwoPoly.features[1] as any,
          ),
        ).toBeLeft();
      });
      it("combines polygons", () => {
        expect(
          addInnerRing(
            fcTwoPolyContainable.features[0] as any,
            fcTwoPolyContainable.features[1] as any,
          ).extract(),
