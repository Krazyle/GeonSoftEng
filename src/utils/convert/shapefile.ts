...
import { type AppError, parseOrError } from "@/utils/errors";
...
async function importAndReproject(obj: Obj) {
  const { importContent } = await import("@/lib/mapshaper/io/mapshaper-import");
  const { exportFileContent } = await import(
    "@/lib/mapshaper/io/mapshaper-export"
  );
  const { projectDataset } = await import(
    "@/lib/mapshaper/commands/mapshaper-proj"
  );
  const { parsePrj } = await import(
    "@/lib/mapshaper/crs/mapshaper-projections"
  );
    ...
    return EitherAsync<ConvertError, ExportResult>(
      async function backShapefile() {
        const { importContent } = await import(
          "@/lib/mapshaper/io/mapshaper-import"
        );
        const { exportFileContent } = await import(
          "@/lib/mapshaper/io/mapshaper-export"
        );
