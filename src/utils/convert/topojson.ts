...
import { ConvertError, parseOrError } from "@/utils/errors";
...
class CTopoJSON implements FileType {
  ...
  forwardString(text: string, _options?: ImportOptions) {
    return EitherAsync<AppError, ConvertResult>(async function forwardTopoJSON({
      liftEither,
      throwE,
    }) {
  ...
  back({ geojson }: { geojson: FeatureCollection }) {
    return EitherAsync<AppError, ExportResult>(async function backTopoJSON({
      throwE,
    }) {
