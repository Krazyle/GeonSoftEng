...
import { z } from "zod";
import { env } from "@/utils/env_client";
import { parseOrError } from "@/utils/errors";
import { GeocodeEarthResult } from "@/utils/geocode";
import type { ZipDB } from "@/utils/get_zip_db";
...
export function getGeocodingURLSearch(
  ...
  if (!text) return null;
  const params = {
    api_key: env.GEOCODE_EARTH_TOKEN,
    text,
  };
...
export function castRowPolyline(
  ...
  return Either.encase(() =>
    polyline.toGeoJSON(String(parsedRow[geometryHeader])),
  ).caseOf({
    Left() {
      return null;
    },
