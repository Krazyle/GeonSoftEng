...
import { useRootItems } from "@/features/panels/components/feature_editor/feature_editor_folder/math";
...
function SvgChild({
  ...
  switch (child.type) {
    ...
    case "Feature": {
      ...
      const d = path(rewind(geometry, true));
      if (!d) return null;
...
export function ExportSVGDialog() {
  ...
  const path = useMemo(() => {
    ...
    for (const { feature } of featureMap.values()) {
      ...
      const rewound = rewind(geometry, true);
      fc.features.push({
        type: "Feature",
        geometry: rewound,
