...
import { scaleUnitAtom, TabOption, tabAtom } from "@/stores/jotai";
import { linearGradient } from "@/utils/color";
import { SCALE_UNITS, type ScaleUnit, zScaleUnit } from "@/utils/constants";
...
function LegendTitle({ title }: { title: string }) {
  ...
  return (
    <div className="w-full px-2 pt-2 text-right flex justify-between items-center">
      {title}
      <Button
        variant="quiet"
...
function LegendContainer({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div
      ...
      dark:text-white
      shadow-lg w-48 rounded-2xl p-1"
    >
      {children}
    </div>
  );
...
function getScale(
  ...
  const x = container.clientWidth / 2 - maxWidth / 2;

  if (Number.isNaN(x) || Number.isNaN(y)) {
    return { maxDistance: 0, unit: "meter" };
  }
...
export function Legend() {
  ...
  return (
    <div className="space-y-4 absolute bottom-4 right-10 w-48 flex flex-col items-end">
      <LegendContainer>{legend}</LegendContainer>
      <div className="bg-gray-50/80 dark:bg-gray-800/80 backdrop-blur-sm p-2 rounded-full shadow-sm">
        <ScaleControl />
      </div>
    </div>
  );
