...
export function ArcCollection() {
  ...
  function initXYData(nn, xx, yy) {
    var size = nn.length;
    if (Array.isArray(nn)) nn = new Uint32Array(nn);
    if (Array.isArray(xx)) xx = new Float64Array(xx);
    if (Array.isArray(yy)) yy = new Float64Array(yy);
    _xx = xx;
    _yy = yy;
    _nn = nn;
    _zz = null;
  ...
  function initZData(zz) {
    if (!zz) {
      ...
      if (Array.isArray(zz)) zz = new Float64Array(zz);
      _zz = zz;
...
function dedupArcCoords(src, dest, arcLen, xx, yy, zz) {
  ...
  while (n < arcLen) {
    ...
    y = yy[j];
    keep =
      x === x && y === y && (n2 === 0 || x !== xx[j - 1] || y !== yy[j - 1]);
    if (keep) {
      i = dest + n2;
      xx[i] = x;
      yy[i] = y;
