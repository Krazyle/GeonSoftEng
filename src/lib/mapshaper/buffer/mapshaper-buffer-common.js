...
function _dissolveBufferDataset(dataset, optsArg) {
  var opts = optsArg || {};
  ...
  var shapes2 = lyr.shapes.map(function (_shp, shapeId) {
...
function _getBufferToleranceFromCircleSegments(n) {
  return 1 - Math.cos(Math.PI / n);
}

function _getArcDegreesFromTolerancePct(pct) {
  return (360 * Math.acos(1 - pct)) / Math.PI;
}

function _getBufferToleranceFromCircleSegments2(n) {
  return 1 / Math.cos(Math.PI / n) - 1;
}

function _getArcDegreesFromTolerancePct2(pct) {
...
function _getBufferToleranceFunction(dataset, opts) {
...
export function getBufferDistanceFunction(lyr, dataset, opts) {
  ...
  var expr = compileValueExpression(opts.radius, lyr, null, {});
