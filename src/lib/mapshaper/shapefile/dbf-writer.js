...
function exportRecords(records, encoding, fieldOrder) {
  ...
  var fieldBuffers = encodeFieldNames(fieldNames, headerEncoding);
  var fieldData = fields.map(function (name, _i) {
...
function initStringField(info, arr, name, encoding) {
  var formatter =
    encoding === "ascii"
      ? encodeValueAsAscii
      : getStringWriterEncoded(encoding);
  var size = 0;
  var truncated = 0;
...
function _cleanFieldName_v1(name) {
