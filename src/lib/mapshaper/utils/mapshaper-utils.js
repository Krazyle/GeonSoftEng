...
function _reduceAsync(arr, memo, iter, done) {
...
function _forEachProperty(o, func, ctx) {
...
function _formatNumberForDisplay(num, decimals, nullStr, showPos) {
  var fmt;
  if (Number.isNaN(num)) {
    fmt = nullStr || "-";
  } else {
...
function _getSortedIds(arr, asc) {
...
function _reorderArray(arr, idxs) {
  var len = idxs.length;
  var arr2 = [];
  for (var i = 0; i < len; i++) {
    var idx = idxs[i];
...
export function findRankByValue(arr, value) {
  if (Number.isNaN(value)) return arr.length;
  var rank = 1;
  for (var i = 0, n = arr.length; i < n; i++) {
    if (value > arr[i]) rank++;
  }
...
function _findValueByPct(arr, pct) {
...
function formatValue(val, matches) {
  ...
  var decimals = matches[3] ? parseInt(matches[3].substr(1), 10) : void 0;
  var type = matches[4];
  var isString = type === "s",
    isHex = type === "x" || type === "X",
    _isInt = type === "d" || type === "i",
    _isFloat = type === "f",
    isNumber = !isString;
...
function _expandoBuffer(constructor, rate) {
  var capacity = 0,
    k = rate >= 1 ? rate : 1.2,
    buf;
...
export function uniqifyNames(names, formatter) {
  ...
  names.forEach(function (name) {
    var i = 0,
      ...
      versionedName;
    while (
      names2.indexOf(candidate) > -1 ||
      (candidate === name && counts[candidate] > 1) ||
