...
function _insertFieldValues(lyr, fieldName, values) {
...
function _getLayerDataTable(lyr) {
  var data = lyr.data;
  if (!data) {
...
function _layerHasNonNullData(lyr) {
...
function _deleteFeatureById(lyr, i) {
...
function _requireSinglePointLayer(lyr, msg) {
...
function _requirePolylineLayer(lyr, msg) {
...
function _getOutputLayer(src, opts) {
  return opts?.no_replace ? { geometry_type: src.geometry_type } : src;
}
...
export function setOutputLayerName(dest, src, defName, opts) {
  ...
  if (opts.name) {
    ...
    dest.name = src?.name || defName || undefined;
  }
}
...
function _filterPathLayerByArcIds(pathLyr, arcIds) {
...
function _isolateLayer(layer, dataset) {
